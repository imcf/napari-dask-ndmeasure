"""napari-chunked-regionprops — out-of-core regionprops for huge labelled images.

Measures Labels layers directly from their backing dask/zarr arrays via a
chunk-local map + merge (see :mod:`napari_chunked_regionprops._measure`), so a
...
try:
    __version__ = _pkg_version("napari-chunked-regionprops")
